...
import { NetworkNode, SimulationEvent, NetworkLink } from '../types/simulation';
import { cn } from '../lib/utils';
import { X, ShieldAlert, Activity, Target, Zap, Cpu, Shield, ChevronDown, ShieldCheck } from 'lucide-react';
import { useEffect, useState } from 'react';
import { getThreatIntelligence } from '../lib/gemini';
...
export function NodeDetails({ 
  ...
  useEffect(() => {
    if (node) {
      ...
      getThreatIntelligence(node.id, node.status, node.type, node.label, fullContext).then(res => {
        setAnalysis(res);
        setLoading(false);
      });
    }
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
              ...
              <div className="h-40 w-full bg-surface/30 rounded-sm border border-border/50">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
                    ...
                    <PolarAngleAxis dataKey="subject" tick={{ fill: '#5A7FA8', fontSize: 9, fontBold: '700' }} />
                    <Radar
                      name="Metrics"
                      dataKey="A"
                      stroke="#00FFD1"
        ...
        <div className="space-y-3">
          <div className={cn(
            ...
            <span className="text-[12px] font-display font-black uppercase tracking-[2px] relative z-10">Sentinel Deep Analysis</span>
            {loading && <div className="absolute inset-0 bg-accent-cyan/10 blur-xl rounded-full scale-110" />}
            {loading && <div className="absolute inset-x-0 -bottom-1 h-[1px] bg-accent-cyan animate-pulse shadow-[0_0_8px_#00FFD1]" />}
          </div>
          <div className="p-4 rounded-sm bg-panel border-l-2 border-accent-cyan min-h-[140px] relative overflow-hidden group">
            {loading ? (
              <div className="space-y-2">
                ...
                <div className="h-3 w-4/6 bg-accent-cyan/10 animate-pulse rounded" />
              </div>
            ) : (
              <p className="text-[12px] text-white/80 leading-[1.8] font-body uppercase">
                {analysis || "Awaiting intelligence feed from core processor..."}
              </p>
            )}
